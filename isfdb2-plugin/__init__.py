...
class ISFDB(Source):
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        ...
        if book_url_tuple:
            ...
            month = identifiers.get('month', None)
            year = identifiers.get('year', None)
            price = identifiers.get('price', None)
            ...
            if isbn or catalog_id:
                ...
                query = getattr(PublicationsList, query_function)(isbn or catalog_id, month, year, price)
            ...
            # If we haven't reached the maximum number of results, also search by title and author, if either is provided
            if len(matches) < self.prefs["max_results"] and self.prefs["search_publications"] and (title or author):
                query = PublicationsList.url_from_title_and_author(title, author, month, year, price)
                stubs = PublicationsList.from_url(self.browser, query, timeout, log)
            ...
            # If we still haven't found enough results, also search *titles* by title and author, if either is provided
            if len(matches) < self.prefs["max_results"] and self.prefs["search_titles"] and (title or author):
                query = TitleList.url_from_title_and_author(title, author, month, year)
